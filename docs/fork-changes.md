...
| `ainode/core/units.py` | One unit for memory: decimal GB everywhere the planner compares, with the memory guard's GiB thresholds converted explicitly at the boundary | Weights were decimal and budgets were binary, both labelled GB — 7.4% of every node, taken out of the KV cache |
...
| `ainode/models/download_retry.py`, `models/resume.py` | Retries a transfer per file, and resumes one by checking every local file against the size the Hub reports rather than trusting the cache's own idea of finished | An unstable link ended a 160 GB pull on one timeout, and what it left behind was listed as On disk |
| `ainode/engine/engine_env.py` | Asks the engine image which `VLLM_*` variables it actually reads, and says so before a launch rather than in one mid-log warning | A recipe knob the image does not know is silently inert, and the only evidence is `Unknown vLLM environment variable detected` between a Triton notice and a NCCL banner |
| `ainode/models/completeness.py`, `models/import_routes.py` | Answers "is this checkpoint all of it" from the shard index, the tokenizer files and the staging files an interrupted transfer leaves — and brings a model in by hand, from the browser or from `/model-import` on the head | A download interrupted by a restart looked finished until an engine failed on a missing shard; and a link that drops cannot carry 86 GB, so the files have to be allowed to arrive by another route |
...
Tests: **88 new files**, 13 upstream files extended, 3053 tests. Two of them
