...
class EugrBackend(EngineBackend):
    ...
    def _build_env(self) -> dict:
        ...
        env = os.environ.copy()
        # A HOST path, deliberately: the launcher turns HF_HOME into a docker
        # bind-mount source, and the Docker daemon resolves mount sources on
        # the host — not inside this container. Passing our own view of the
        # path mounted the host's /root/.ainode/models (root's home) instead of
        # the operator's, so the engine's Hugging Face cache pointed at an
        # empty directory and refetched metadata on every launch.
        env.setdefault(
            "HF_HOME", host_path(self.config.models_dir or "/root/.ainode/models"))
    ...
    def _serve_target_and_name(self) -> tuple:
        """What to hand ``vllm serve``, and the API id to serve it under.

        A model already on disk is served from its directory, not from its repo
        id. The two are not equivalent: given a repo id, vLLM resolves it
        through the Hugging Face cache — and AINode's downloader writes a flat
        ``org--name`` directory, not the ``models--org--name`` layout that
        cache uses. So a model the operator downloaded through the UI, sitting
        right there on the disk mounted into the container, was invisible to
        the engine, which then pulled all 23 GB of it again on every single
        launch. That is the "loading takes forever" this fixes; the NVIDIA
        backend has served from disk for this reason all along.

        Serving from a path renames the model in the API, so the repo id is
        restored with ``--served-model-name``. An alias the caller set wins
        over both.
        """
        model = (self.config.model or "").strip()
        if not model:
            return "", ""
        local = local_model_dir(model, self.config.models_dir or "")
        if not local:
            return model, ""
        # The path as the ENGINE container sees it: models_dir is mounted at
    ...
    def _write_launch_script(self, plan: ParallelPlan, solo: bool = False) -> Path:
        ...
        Every interpolated value is shell-quoted, and that is not a precaution
        — it is a bug fix. This writes a *bash script*, so an argument
        containing braces, quotes or spaces is rewritten by the shell before
        vLLM ever sees it. A speculative config is the case that bites:

            --speculative_config {"method":"qwen3_5_mtp","num_speculative_tokens":2}

        is brace expansion. Bash splits it at the comma and strips the quotes,
        and vLLM receives two words, the first being `method:qwen3_5_mtp` —
        ...
        if self.config.quantization and wanted("--quantization"):
            extra += f"    --quantization {shlex.quote(str(self.config.quantization))} \\\n"
        ...
        if wanted("--kv-cache-dtype"):
            ...
            if kv_dtype:
                extra += f"    --kv-cache-dtype {shlex.quote(str(kv_dtype))} \\\n"
        ...
        if not names and implied_name:
            # Serving from a path would otherwise publish the path as the
            # model id, and every client addressing the repo id would 404.
            names = [implied_name]
        if names and wanted("--served-model-name"):
            extra += ("    --served-model-name "
